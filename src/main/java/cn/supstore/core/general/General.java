package cn.supstore.core.general;
...
//        		.collect(Collectors.toMap(p -> p.getKey(), p -> ""+redisTemplate.boundValueOps(p.getKey()).get())
    ...
    @RequestMapping(value = "/scheduler/{key}/unlock",method = RequestMethod.POST)
    @ResponseBody
    public ResponseEntity schedulerUnlock(Model model, @PathVariable String key){
    		if(key!=null) {
